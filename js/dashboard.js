...
function buildDashboard() {
  ...
  var totalPjum  = (function() {
    var s = {}; pjum.forEach(function(r) { if(r[P.file]) s[r[P.file]] = 1; });
    return Object.keys(s).length;
  })();
  var totalCost = pjum.reduce(function(s, r) { return s + (parseFloat(r[P.jumlah]) || 0); }, 0);
  var totalDesa = (function() {
    var s = {}; benef.forEach(function(r) { if(r[B.desa]) s[r[B.desa]] = 1; });
    return Object.keys(s).length;
  })();
  var totalKab = (function() {
    var s = {}; benef.forEach(function(r) { if(r[B.kab]) s[r[B.kab]] = 1; });
    return Object.keys(s).length;
  })();
  var uniqBenefSet = {};
  benef.forEach(function(r) {
    uniqBenefSet[(r[B.nama]||'').toLowerCase() + '|' + (r[B.desa]||'')] = 1;
  });
  var uniqBenef = Object.keys(uniqBenefSet).length;
  ...
  var benefByBulan = sortedBulan(groupCount(benef, function(r) { return r[B.tgl] ? r[B.tgl].slice(0, 7) : null; }));
  var bLabels = benefByBulan.map(function(x) {
    var parts = x[0].split('-'); return bulanName(parts[1]) + "'" + parts[0].slice(2);
  });
  mkLine('ch-dash-benef-trend', bLabels, benefByBulan.map(function(x) { return x[1]; }),
    '#F97316', { label: 'Beneficiary', noLegend: true });

  /* ── Distribusi Jenis Benef ── */
  ...
  mkDonut('ch-dash-benef-donut', katData.map(function(x) { return x[0]; }), katData.map(function(x) { return x[1]; }));
  ...
  var kabTotal = benef.length || 1;
  renderRankList('rank-kab', kabData, kabMax, kabTotal, function(v) { return v.toLocaleString(); }, false);
  ...
  renderRankList('rank-kegiatan', kegData, kegMax, kegTotal, function(v) {
  ...
  renderRankList('rank-benefit', benData, benMax, kegTotal, function(v) {
    return v.toLocaleString() + ' (' + (v / kegTotal * 100).toFixed(1) + '%)';
  }, true);
...
function buildAiInsight(benef, pjum) {
  ...
  var tbParts  = (topBulanCost[0] || '-').split('-');
  var topBulan = tbParts[1] ? bulanFull(tbParts[1]) + ' ' + tbParts[0] : '—';
...
function renderRankList(id, entries, max, total, valFmt, showBar) {
  var el = document.getElementById(id);
  if (!el) return;
  el.innerHTML = entries.map(function(x, i) {
    var barHtml = showBar
    ...
    return '<div class="rank-item cols3">' +
      ...
      '<div><div class="rank-name">' + x[0] + '</div>' + barHtml + '</div>' +
